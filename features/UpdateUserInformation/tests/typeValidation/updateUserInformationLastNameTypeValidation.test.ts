...
describe("test user information update feature last name type validation", () => {
    test("Cannot update a user using an array as a last name", async () => {
        ...
        const misformedRequest = () => {
            updateUserInformationRequest.setLastName(arrayInsteadOfStringInput as unknown as string);
        }

        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Last name needs to be of type \"string\", \"array\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("last_name_type_is_not_string_but_array");
        }
    });
    ...
    test("Cannot update a user using an object as a last name", async () => {
        ...
        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Last name needs to be of type \"string\", \"object\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("last_name_type_is_not_string_but_object");
        }
    });
    ...
    test("Cannot update a user using null as a last name", async () => {
        ...
        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Last name needs to be of type \"string\", \"null\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("last_name_type_is_not_string_but_null");
        }
    });
    ...
    test("Cannot update a user using a boolean as a last name", async () => {
        ...
        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Last name needs to be of type \"string\", \"boolean\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("last_name_type_is_not_string_but_boolean");
        }
    });
    ...
    test("Cannot update a user using undefined as a last name", async () => {
        ...
        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Last name needs to be of type \"string\", \"undefined\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("last_name_type_is_not_string_but_undefined");
        }
    });
    ...
    test("Cannot update a user using a number as a last name", async () => {
        ...
        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Last name needs to be of type \"string\", \"number\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("last_name_type_is_not_string_but_number");
        }
    });
    ...
    test("Cannot update a user using a function as a last name", async () => {
        ...
        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Last name needs to be of type \"string\", \"function\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("last_name_type_is_not_string_but_function");
        }
    });
    ...
    test("Cannot update a user using a symbol as a last name", async () => {
        ...
        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Last name needs to be of type \"string\", \"symbol\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("last_name_type_is_not_string_but_symbol");
        }
    });
});