...
describe("test user registration feature username type validation", () => {
    test("Cannot update a user using an array as a username", async () => {
        ...
        const misformedRequest = () => {
            registerUserRequest.setUsername(arrayInsteadOfStringInput as unknown as string);
        }

        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Username needs to be of type \"string\", \"array\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("username_type_is_not_string_but_array");
        }
    });
    ...
    test("Cannot update a user using an object as a username", async () => {
        ...
        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Username needs to be of type \"string\", \"object\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("username_type_is_not_string_but_object");
        }
    });
    ...
    test("Cannot update a user using null as a username", async () => {
        ...
        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Username needs to be of type \"string\", \"null\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("username_type_is_not_string_but_null");
        }
    });
    ...
    test("Cannot update a user using a boolean as a username", async () => {
        ...
        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Username needs to be of type \"string\", \"boolean\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("username_type_is_not_string_but_boolean");
        }
    });
    ...
    test("Cannot update a user using undefined as a username", async () => {
        ...
        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Username needs to be of type \"string\", \"undefined\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("username_type_is_not_string_but_undefined");
        }
    });
    ...
    test("Cannot update a user using a number as a username", async () => {
        ...
        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Username needs to be of type \"string\", \"number\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("username_type_is_not_string_but_number");
        }
    });
    ...
    test("Cannot update a user using a function as a username", async () => {
        ...
        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Username needs to be of type \"string\", \"function\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("username_type_is_not_string_but_function");
        }
    });
    ...
    test("Cannot update a user using a symbol as a username", async () => {
        ...
        expect(misformedRequest).toThrow(BadRequestError);
        expect(misformedRequest).toThrow("Username needs to be of type \"string\", \"symbol\" given");

        try {
            await misformedRequest();
        } catch(error: any) {
            expect(error.getErrorCode()).toBe("username_type_is_not_string_but_symbol");
        }
    });
});